...
async function bootstrap() {
  try {
    ...
    logger.info('🚀 Starting RSS Skull Bot v0.01 (VERBOSE MODE)...');
    console.log('🚀 Starting RSS Skull Bot v0.01 (VERBOSE MODE)...');
    ...
    const initPromise = botService.initialize();
    ...
    if (error instanceof Error && error.message.includes('timeout')) {
      logger.error('💡 Bot initialization timed out. This could be due to:');
      logger.error('   • Network connectivity issues to Telegram API');
      logger.error('   • Bot token problems');
      logger.error('   • Webhook conflicts');
      logger.error('   • Database or Redis connection issues');
      console.error('💡 Bot initialization timed out - check network connectivity');
    }
