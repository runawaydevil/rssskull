...
def sanitize_html_for_telegram(text: str) -> str:
    ...
    text = re.sub(r'</?strong>', lambda m: m.group(0).replace('strong', 'b'), text, flags=re.IGNORECASE)
    ...
    text = re.sub(r'</?strike>', lambda m: m.group(0).replace('strike', 's'), text, flags=re.IGNORECASE)
    ...
    def clean_a_tag(match):
        ...
        if href_match:
            ...
            # Don't escape href URL - it must remain valid for links
